  ...
  useEffect(() => {
    ...
    if (savedMessages) {
      ...
      const mockMessages = [
        {
          id: '1',
          text: 'És uma pessoa incrível! Continua assim 💜',
          timestamp: new Date(Date.now() - 1000 * 60 * 30).toISOString(),
          read: false
        },
        {
          id: '2',
          text: 'Admiro muito a tua dedicação aos estudos',
          timestamp: new Date(Date.now() - 1000 * 60 * 60 * 2).toISOString(),
          read: false
        }
      ]
      setMessages(mockMessages)
      localStorage.setItem(`kala_messages_${parsedUser.uid}`, JSON.stringify(mockMessages))
    }
  }, [])
